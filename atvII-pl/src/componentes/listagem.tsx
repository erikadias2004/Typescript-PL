...
export default class Listagem extends Component {
    stateValor = {
        clientesValor: Array.from({ length: 5 }, (_, i) => `cliente ${i + 1}`)
    };
    stateQuantidade = {
        clientesQuantidade: Array.from({ length: 10 }, (_, i) => `cliente ${i + 1}`)
    }
    stateProdutos = {
        produtos: Array.from({ length: 10 }, (_, i) => `produto ${i + 1}`)
    }
    stateServicos = {
        servicos: Array.from({ length: 10 }, (_, i) => `servico ${i + 1}`)
    }

    render() {
        const { clientesValor } = this.stateValor;
        const { clientesQuantidade } = this.stateQuantidade;
        const { produtos } = this.stateProdutos;
        const { servicos } = this.stateServicos;
